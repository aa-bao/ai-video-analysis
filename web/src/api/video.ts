...
/** 从本机浏览器自动导出抖音 Cookie（默认直接写入共享文件） */
export interface ExtractDouyinResult extends VideoCookieStatus {
  written?: boolean
  source?: string
  unsupported_v20?: number
  failed_decrypt?: number
}

export async function extractDouyinCookies(
  browser?: string,
): Promise<ExtractDouyinResult> {
  const resp = await client.post<ExtractDouyinResult>('/video/cookies/douyin/extract', {
