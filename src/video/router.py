...
from src.core.security import require_permission, _session_factory
...
from src.video.acquire import validate_public_url
from src.video.cookies import get_cookie_status, save_douyin_cookie, save_yuanbao_cookie
from src.video.cookie_extract import CookieExtractError, extract_douyin_cookie
...
require_user = require_permission(PERMISSION_PROJECT_VIEW)
...
class ExtractDouyinCookieRequest(BaseModel):
    model_config = {"extra": "forbid"}
    # 空 = 自动挑第一个有抖音 Cookie 的浏览器库
    browser: str | None = Field(default=None, max_length=64)
    # True = 只导出不写入（用户可先预览再决定）
    dry_run: bool = False
...
async def _owned_task_state(
    request: Request, task_id: str, principal: RequestIdentity
) -> dict[str, object]:
    async with request.app.state.session_factory() as session:
        record = await VideoTaskRepository(session).get_owned(task_id, principal)
    if record is None:
        raise AppError("VIDEO_TASK_NOT_FOUND", "Task not found", status_code=404)
    state = VideoTaskRepository.record_to_state(record)
    manager = _manager(request)
    if manager.get(task_id) is None:
        manager.restore_cache(state)
    return state


def _owner_upload_dir(manager: VideoTaskManager, principal: RequestIdentity) -> Path:
    # 目录按归属键分桶（账号 u:<id> / 访客 v:<visitor_id>），替代原 tenant 分桶
    owner_key = hashlib.sha256(principal.owner_key.encode("utf-8")).hexdigest()[:16]
    return manager._config.upload_dir / owner_key / str(principal.user_id)


def _owned_upload_source(
    manager: VideoTaskManager, source: str, principal: RequestIdentity
) -> Path:
    path = Path(source).expanduser().resolve()
    owner_root = _owner_upload_dir(manager, principal).resolve()
    if not path.is_relative_to(owner_root) or not path.is_file():
        raise AppError("VIDEO_FILE_NOT_FOUND", "Uploaded file not found", status_code=404)
    return path


def _safe_artifact_path(manager: VideoTaskManager, raw: str | Path) -> Path:
    path = Path(raw).expanduser().resolve()
    root = manager._config.output_root.resolve()
    if not path.is_relative_to(root):
        raise AppError("VIDEO_FILE_NOT_FOUND", "Task artifact is outside persistent storage", status_code=404)
    return path


def _safe_library_artifact_path(manager: VideoTaskManager, raw: str | Path) -> Path:
    path = Path(raw).expanduser().resolve()
    roots = (
        manager._config.output_root.resolve(),
        manager._config.library_root.resolve(),
    )
    if not any(path != root and path.is_relative_to(root) for root in roots):
        raise AppError(
            "VIDEO_FILE_NOT_FOUND",
            "Library artifact is outside persistent storage",
            status_code=404,
        )
    return path


def _safe_task_images(manager: VideoTaskManager, items: object) -> list[dict]:
    safe: list[dict] = []
    for item in items if isinstance(items, list) else []:
        if not isinstance(item, dict) or not item.get("path"):
            continue
        try:
            path = _safe_artifact_path(manager, str(item["path"]))
        except AppError:
            continue
        if path.is_file():
            safe.append({**item, "path": str(path)})
    return safe


def _library_view(state: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "title": str(report.get("title") or state["task_id"]),
        "summary": str(summary.get("summary") or ""),
        "keypoints": list(summary.get("keypoints") or []),
        "transcript": str(state.get("transcript") or ""),
        "keyframes": list(state.get("keyframes") or []),
        "has_report": bool(output_dir and (Path(output_dir) / "report.html").is_file()),
        "created_at": state.get("created_at"),
        "source": str(state.get("source") or ""),
        "duration_seconds": state.get("duration_seconds") or report.get("duration_seconds"),
        "cost": state.get("cost"),
        "source_kind": VideoTaskManager._classify_source(str(state.get("source") or "")),
...
def _task_media_path(
    ...
    if raw:
        ...
        roots = (manager._config.output_root.resolve(), manager._config.upload_dir.resolve())
        if p.is_file() and any(p.resolve().is_relative_to(root) for root in roots):
            return p
    ...
    if output_dir:
        ...
        for suffix in output_suffixes:
            for p in output.glob(f"*{suffix}"):
                if p.is_file() and p.resolve().is_relative_to(manager._config.output_root.resolve()):
                    return p
    return None
...
async def get_disk_status(
    ...
    """只读：视频产物文件系统当前用量(%)与告警级别（方案 7.3 磁盘专项）。"""
    manager = _manager(request)
    return {"success": True, "data": manager.disk_status()}
...
async def submit_task(
    ...
    if body.kind == "file":
        p = _owned_upload_source(manager, source, principal)
        source = str(p)
        if not p.exists():
        ...
        try:
            ...
            raise AppError("VIDEO_BAD_URL", str(exc), status_code=400) from exc

    # 任务受理前 fail-closed：视频产物硬配额 + 文件系统用量告警（方案 7.3 磁盘专项）。
    # 必须在 submit 之前 —— 拒绝时不创建任何产物目录 / 任务记录（属于「未受理」）。
    manager.check_artifact_quota()
    manager.check_disk_alert()

    # 阶段 6：访客「解析条数」配额 fail-closed 受理前校验（403，绝不 401）。
    if principal.is_visitor:
        await VisitorTaskQuota(db, principal).check_quota()

    state = manager.submit(
        source,
        owner_user_id=principal.user_id,
        owner_visitor_id=principal.visitor_id if principal.is_visitor else None,
        is_visitor=principal.is_visitor,
        kind=body.kind,
        frames=body.frames,
    ...
    try:
        await manager.persist_to_db(state, strict=True)
    except Exception as exc:
        manager._state_path(str(state["task_id"])).unlink(missing_ok=True)
        raise AppError(
            "VIDEO_DATABASE_UNAVAILABLE",
            "Video task database is unavailable",
            status_code=503,
        ) from exc
...
async def upload_video(
    ...
    upload_dir = _owner_upload_dir(manager, principal)
...
@router.get("/asr-media/{token}", include_in_schema=False)
async def get_dashscope_asr_media(token: str) -> FileResponse:
    """Serve a short-lived capability URL for DashScope file transcription."""
    if not re.fullmatch(r"[a-f0-9]{64}", token):
        raise AppError("VIDEO_ASR_MEDIA_NOT_FOUND", "Media not found", status_code=404)
    path = video_asr.resolve_dashscope_media(token)
    if path is None:
        raise AppError("VIDEO_ASR_MEDIA_NOT_FOUND", "Media not found", status_code=404)
    return FileResponse(
        str(path),
...
async def get_video_settings(
    request: Request,
    principal: RequestIdentity = Depends(require_permission(PERMISSION_SETTINGS_MANAGE)),
) -> dict[str, object]:
...
@router.post("/cookies/douyin/extract")
async def extract_douyin_cookie_endpoint(
    body: ExtractDouyinCookieRequest,
    request: Request,
    principal: RequestIdentity = Depends(require_permission(PERMISSION_SETTINGS_MANAGE)),
) -> dict[str, object]:
    """从本机浏览器**自动导出**抖音 Cookie 并（默认）直接写入共享文件。

    抖音登录态过期后，原先必须手动从浏览器里抠 Cookie 再粘贴。
    这里直接读本机 Chromium 系浏览器的 Cookie 库并转成 Netscape 格式 ——
    用户在浏览器里登录一次即可一键更新。

    ⚠️ Chrome 127+ 的 `v20`（App-Bound 加密）**解不开**，
    此时返回 400 并明确告知需改用手动粘贴；**不要**把它报成「登录失效」。
    `dry_run=True` 时只返回内容不落盘。
    """
    try:
        result = extract_douyin_cookie(body.browser)
    except CookieExtractError as exc:
        raise AppError("VIDEO_COOKIE_EXTRACT_FAILED", str(exc), status_code=400) from exc
    ...
    if body.dry_run:
        return {
            ...
            "data": {
                ...
                "unsupported_v20": result.get("unsupported_v20", 0),
                "failed_decrypt": result.get("failed_decrypt", 0),
    ...
    config = _manager(request)._config
    try:
        status = save_douyin_cookie(result["content"], config.douyin_cookie_file)
    except ValueError as exc:
...
async def list_library(
    ...
    async with request.app.state.session_factory() as session:
        records = await VideoTaskRepository(session).list_owned(principal, limit=200)
    tasks = [
        _library_view(VideoTaskRepository.record_to_state(record))
...
async def delete_library_task(
    ...
    """Delete an owned library task and its persistent artifacts."""
    manager = _manager(request)
    state = await _owned_task_state(request, task_dir, principal)
    await manager.cancel_task(task_dir)
    output_dir = state.get("output_dir")
    if output_dir:
        target = _safe_library_artifact_path(manager, str(output_dir))
        if target.is_dir():
            shutil.rmtree(target)
    async with request.app.state.session_factory() as session:
        deleted = await VideoTaskRepository(session).delete_owned(task_dir, principal)
    if not deleted:
        raise AppError("VIDEO_TASK_NOT_FOUND", "Task not found", status_code=404)
    return {"success": True, "data": {"deleted": task_dir}}
...
async def list_tasks(
    ...
    async with request.app.state.session_factory() as session:
        records = await VideoTaskRepository(session).list_owned(principal, limit=100)
    tasks = [VideoTaskRepository.record_to_state(record) for record in records]
    return {"success": True, "data": tasks}
...
async def get_task(
    ...
    state = await _owned_task_state(request, task_id, principal)
    return {"success": True, "data": state}
...
async def get_task_events(
    ...
    manager = _manager(request)
    await _owned_task_state(request, task_id, principal)
...
async def delete_task(
    ...
    if state.get("output_dir"):
        output_dir = _safe_artifact_path(manager, str(state["output_dir"]))
        if output_dir.is_dir():
            shutil.rmtree(output_dir)
    async with request.app.state.session_factory() as session:
        deleted = await VideoTaskRepository(session).delete_owned(task_id, principal)
    if not deleted:
        raise AppError("VIDEO_TASK_NOT_FOUND", "Task not found", status_code=404)
...
async def get_library_frame(
    ...
    await _owned_task_state(request, task_dir, principal)
...
async def get_frame(
    ...
    output_dir = state.get("output_dir")
    if output_dir:
        candidate = _safe_artifact_path(manager, str(output_dir)) / "frames" / safe
        if candidate.exists():
            frame_path = candidate
    if frame_path is None:
        for kf in state.get("keyframes") or []:
            try:
                p = _safe_artifact_path(manager, str(kf.get("path", "")))
            except AppError:
                continue
...
async def get_post_image(
    ...
    output_dir = state.get("output_dir")
    if output_dir:
        candidate = _safe_artifact_path(manager, str(output_dir)) / "images" / safe
        if candidate.exists():
            image_path = candidate
    if image_path is None:
        for img in state.get("post_images") or []:
            try:
                p = _safe_artifact_path(manager, str(img.get("path") or ""))
            except AppError:
                continue
            if p.name == safe and p.exists():
                image_path = p
...
async def get_task_video(
    ...
    manager = _manager(request)
    state = await _owned_task_state(request, task_id, principal)
    video_path = _task_media_path(manager, state, "video_path", (".mp4", ".webm", ".mov", ".mkv", ".m4v"))
    if video_path is None and state.get("kind") == "file":
        # 兼容历史本地文件任务：源文件仍然存在时可直接播放
        candidate = _owned_upload_source(manager, str(state.get("source") or ""), principal)
...
async def get_task_audio(
    ...
    manager = _manager(request)
    state = await _owned_task_state(request, task_id, principal)
    audio_path = _task_media_path(manager, state, "audio_path", (".mp3", ".m4a", ".aac"))
...
async def get_report_html(
    ...
    html_path = _safe_artifact_path(manager, str(output_dir)) / "report.html"
...
async def _require_complete_task(
    request: Request, task_id: str, principal: RequestIdentity
) -> dict:
    state = await _owned_task_state(request, task_id, principal)
...
async def ask_question(
    ...
    manager = _manager(request)
    state = await _require_complete_task(request, task_id, principal)
    settings = _settings(request)
    history = manager.get_qa_history(task_id)
    context = _qa_context_text(state)
    keyframes = _safe_task_images(manager, state.get("keyframes"))
    post_images = _safe_task_images(manager, state.get("post_images"))
