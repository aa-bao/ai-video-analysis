...
from src.video.quota import ARTIFACT_QUOTA_EXCEEDED, ArtifactUsage, DiskAlertMonitor
...
# 每任务最大运行时长（方案 §8.2 阶段 5 第 5 条 / D14 定案：1800 → 3600）
_TASK_TIMEOUT_SECONDS = 3600

# 单条视频源时长上限（方案 §8.2 阶段 5 第 5 条）：超过**直接拒绝**，不截断。
# 依据：2C4G / 60GB 盘 / 5 Mbps 规格下，单条 20 分钟视频产物约 200–500 MB（方案 §7.3），
# 且 ASR 按时长计费 —— 超长任务会同时吃满带宽、磁盘与模型额度。
MAX_TASK_DURATION_SECONDS = 1200

# 「未受理」的稳定错误码。方案 §11.7 要求「超时长等『未受理』失败不扣次数」，
# 阶段 6 的配额闸门按此码判断，因此现在就写进状态与事件。
REJECT_DURATION_LIMIT = "DURATION_LIMIT_EXCEEDED"
...
class VideoTaskRejected(VideoTaskError):
    """任务**未受理**（超时长等）。

    与 `VideoTaskError` 的区别不是措辞：前者是"处理过程中失败"，本类是"根本不该受理"。
    方案 §11.7 规定这类失败在配额计数上**不扣次数**（阶段 6 按 `code` 判定）。
    """

    def __init__(self, message: str, *, code: str = REJECT_DURATION_LIMIT) -> None:
...
class _TaskRuntime:
    ...
    pending_frames: asyncio.Task | None = None
...
class VideoTaskManager:
    ...
    def __init__(
        ...
        # 视频产物磁盘硬配额与文件系统告警（方案 7.3 / 11.4）
        self._artifact_usage = ArtifactUsage(
            [config.task_root, config.output_root, config.upload_dir]
        )
        self._disk_alert = DiskAlertMonitor(
            config.output_root,
            warn_percent=config.disk_warn_percent,
            critical_percent=config.disk_critical_percent,
        )
        # 视频任务全局串行 1（方案 7.3 / 11.7）：容量前提，非优化项
        self._serial_semaphore = asyncio.Semaphore(1)
        self._quota_consumed: set[str] = set()  # 阶段 6 访客配额扣减幂等标记
        self._running: dict[str, asyncio.Task] = {}
        # 事件流：按 task_id 保存内存事件缓冲 + SSE 订阅者队列
        self._event_buffers: dict[str, list[dict[str, Any]]] = {}
        self._subscribers: dict[str, set[asyncio.Queue]] = {}
        self._pending_db_states: dict[str, dict[str, Any]] = {}
        self._persist_workers: dict[str, asyncio.Task] = {}
    ...
    def restore_cache(self, state: dict[str, Any]) -> None:
        """Restore the disposable file cache from the authoritative DB row."""
        self._write_cache(copy.deepcopy(state))
    ...
    def _schedule_persist(self, state: dict[str, Any]) -> None:
        if self._app is None:
            return
        task_id = str(state.get("task_id") or "")
        if not task_id:
            return
        self._pending_db_states[task_id] = copy.deepcopy(state)
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            return
        worker = self._persist_workers.get(task_id)
        if worker is None or worker.done():
            worker = loop.create_task(self._flush_persisted_state(task_id))
            self._persist_workers[task_id] = worker

    async def _flush_persisted_state(self, task_id: str) -> None:
        try:
            while state := self._pending_db_states.pop(task_id, None):
                await self.persist_to_db(state)
        finally:
            self._persist_workers.pop(task_id, None)
            pending = self._pending_db_states.get(task_id)
            if pending is not None:
                self._schedule_persist(pending)

    # ── 数据库镜像（文件系统仍是实时状态源，DB 用于存档/统计） ──

    async def persist_to_db(
        self, state: dict[str, Any] | str, *, strict: bool = False
    ) -> None:
        """把任务状态写入数据库；失败只记日志，不阻断流水线。"""
        if self._app is None:
            return
    ...
    async def recover_incomplete_tasks(self) -> int:
        """Restart DB-recorded work interrupted by a process crash."""
        if self._app is None:
            return 0
        session_factory = getattr(self._app.state, "session_factory", None)
        if session_factory is None:
            return 0
        async with session_factory() as session:
            records = await VideoTaskRepository(session).list_incomplete()
        recovered = 0
        for record in records:
            ...
            self.start_background(
                state["task_id"],
                state["source"],
                frames=int(state.get("frames_requested") or 12),
                kind=str(state.get("kind") or "url"),
                content_type=str(state.get("content_type") or "auto"),
            )
            recovered += 1
        return recovered

    # 磁盘硬配额与告警（方案 7.3 / 11.4）

    def artifact_bytes_used(self, *, force: bool = False) -> int:
        """当前视频产物已用字节数（带 TTL 缓存，避免大目录每次全量遍历）。"""
        return self._artifact_usage.bytes_used(force=force)

    def check_artifact_quota(self) -> None:
        """任务受理前 fail-closed 检查：超过硬配额即拒绝并明确提示。

        调用时点必须在 submit 之前 —— 拒绝时不创建任何产物目录/
        任务记录，且属于未受理拒绝（方案 11.7：磁盘满 / 超时长 / 超体积
        都不扣配额次数）。
        """
        used = self.artifact_bytes_used(force=False)
        quota = self._config.artifact_quota_bytes
        if used >= quota:
            used_gib = used / (1024 ** 3)
            quota_gib = quota / (1024 ** 3)
            raise AppError(
                ARTIFACT_QUOTA_EXCEEDED,
                f"视频产物已用 {used_gib:.1f} GiB，已达硬配额 {quota_gib:.0f} GiB，"
                f"拒绝新任务。请删除或导出部分历史产物后再试。",
                status_code=507,
            )

    def check_disk_alert(self) -> str:
        """对产物文件系统统计使用率并按状态机告警（每任务调用一次即可）。"""
        return self._disk_alert.check()

    def disk_status(self) -> dict:
        """只读：产物文件系统当前用量(%)与告警级别（供 /api/video/disk 端点）。"""
        monitor = self._disk_alert
        return {
            "usage_percent": monitor.usage_percent(),
            "level": monitor.level,
            "warn_percent": monitor.warn_percent,
            "critical_percent": monitor.critical_percent,
        }

    async def _consume_visitor_quota(self, task_id: str) -> None:
        """访客「解析条数」配额扣减（阶段 6 · 方案 §11.7）。

        受理成功后、ASR 之前调用（由 _run_pipeline / _run_image_pipeline 触发）。
        幂等：同一 task_id 只计一次。账号（非访客）/ 无 app / 无访客行 → 直接返回。
        """
        if self._app is None:
            return
        state = self._load(task_id) or {}
        if not state.get("is_visitor"):
            return
        visitor_id = state.get("owner_visitor_id")
        if not visitor_id:
            return
        if task_id in self._quota_consumed:
            return
        session_factory = getattr(self._app.state, "session_factory", None)
        if session_factory is None:
            return
        ...
        identity = RequestIdentity.for_visitor(
            visitor_id, shadow_user_id=state.get("owner_user_id")
        )
        async with session_factory() as session:
            await VisitorTaskQuota(session, identity).check_and_consume_task(task_id)
        self._quota_consumed.add(task_id)
    ...
    async def run(
        ...
        # 全局串行 1：同一时刻只允许一条视频任务真正执行（容量前提，非优化项）。
        await self._serial_semaphore.acquire()
        try:
            ...
            raise
        except VideoTaskRejected as exc:
            # 未受理（超时长）：落 failed，但带稳定错误码供配额侧区分
            await self._fail(task_id, str(exc), code=exc.code)
        except Exception as exc:  # noqa: BLE001 — 任务失败统一落盘
            await self._fail(task_id, f"任务执行异常: {exc}")
        finally:
            self._serial_semaphore.release()
    ...
    async def _run_pipeline(
        ...
        try:
            if resolved_type == "image_text":
                await self._run_image_pipeline(
                    task_id, source, settings, output_root, started, runtime,
                )
            else:
            ...
            frames_task = runtime.pending_frames
        ...
        state["output_dir"] = str(output_root)
        state["report"] = runtime.report
        state["transcript"] = runtime.transcript
        state["keyframes"] = runtime.keyframes
        state["cost"] = runtime.report.get("cost") or {}
        state["summary"] = runtime.summary
        state["transcript_source"] = runtime.transcript_source
        state["video_path"] = state.get("video_path") or runtime.video_path
        state["audio_path"] = state.get("audio_path") or runtime.audio_path
        self._save(state)
        if state.get("content_type") == "image_text":
            ...
            complete_msg = (
                f"已生成转录 {len(runtime.transcript)} 字、"
                f"{len(runtime.keyframes)} 张关键帧、摘要与 HTML 报告。"
            )
            complete_data = {
                "duration_seconds": runtime.report.get("duration_seconds"),
                "cost": runtime.report.get("cost"),
            }
        self._emit_event(
        ...
        try:
            ...
            manifest_out: dict[str, Any] = {
                ...
                "transcript_source": runtime.transcript_source,
                "report": runtime.report,
                "summary": runtime.summary,
                "keyframes": runtime.keyframes,
                "cost": runtime.report.get("cost") or {},
    ...
    async def _run_image_pipeline(
        ...
        # 阶段 6：图文帖无时长检查点，受理即扣减（幂等）。
        await self._consume_visitor_quota(task_id)
    ...
    async def _pipeline_body(
        ...
        with tempfile.TemporaryDirectory(prefix="rag-video-") as tmp:
            ...
            if kind == "url":
                ...
                # 主检查点：字幕元信息已给出时长，且尚未下载音频 —— 拒绝代价最低
                self._enforce_duration_limit(task_id, duration, checkpoint="captions_fetched")
                ...
                if caption_text and duration and not acquire.caption_needs_asr(coverage, _CAPTION_MIN_COVERAGE):
                    ...
                    # 兜底点：字幕元信息没给时长时，下载后立即校验，不再往下走 ASR
                    self._enforce_duration_limit(task_id, duration, checkpoint="audio_downloaded")
                ...
                # 本地文件：时长探测完成后、抽取音频之前校验
                self._enforce_duration_limit(task_id, duration, checkpoint="local_probed")
                ...
                if local_path.suffix.lower() in {".mp4", ".mkv", ".webm", ".mov", ".m4v", ".avi", ".flv", ".wmv"}:
                    runtime.video_path = str(local_path.resolve())
                    live_state = self._load(task_id)
                    if live_state is not None:
                        live_state["video_path"] = runtime.video_path
            ...
            if audio_path is not None:
                ...
                runtime.audio_path = str(audio_dest)
                live_state = self._load(task_id)
                if live_state is not None:
                    live_state["audio_path"] = runtime.audio_path
                    self._save(live_state)

            # 阶段 6：访客「解析条数」配额——所有时长检查点已通过、ASR 之前扣减。
            # 未受理失败（超时长 / 坏链 / DB 不可用）由 run() 捕获，绝不触达此处。
            await self._consume_visitor_quota(task_id)
            ...
            if frames > 0 and duration:
                ...
                runtime.pending_frames = frames_task
        ...
        runtime.summary = summary
        runtime.transcript_source = transcript_source
    ...
    async def _extract_frames(
        ...
        try:
            ...
            if acquire.is_url(source):
                ...
                runtime.video_path = str(video_for_frames.resolve())
                live_state = self._load(task_id)
                if live_state is not None:
                    live_state["video_path"] = runtime.video_path
                    self._save(live_state)
                self._emit_event(
                    ...
                    level="success",
                    data={"video_path": runtime.video_path},
                )
            else:
    ...
    async def _fail(self, task_id: str, message: str, *, code: str | None = None) -> None:
        state = self._load(task_id)
        if state is None:
            return
        ...
        state["error"] = message
        if code:
            # 稳定错误码：阶段 6 的配额闸门靠它区分「未受理」与「处理失败」
        ...
        self._emit_event(
            ...
            message or "未知错误",
            level="error",
            data={"error": message, **({"error_code": code} if code else {})},
        )
        self._close_event_stream(task_id)
        await self.persist_to_db(task_id)
    ...
    def _enforce_duration_limit(self, task_id: str, duration: float, *, checkpoint: str) -> None:
        """源时长超过 20 分钟直接拒绝（方案 §8.2 阶段 5 第 5 条）。

        三个调用点，按代价从低到高排列：

        1. ``captions_fetched`` —— **主检查点**：平台字幕元信息就能给出时长，
           此时**尚未下载任何音频**，拒绝代价最低；
        2. ``audio_downloaded`` —— 兜底（字幕元信息里没有时长）；
        3. ``local_probed`` —— 本地文件：时长探测完成后、抽取音频之前。

        这里是**拒绝**而不是截断（依方案原文「超长直接拒绝」）：截断会让用户拿到
        一份被悄悄切短的摘要，比明确拒绝更糟 —— 用户无从知道结论少了一半。
        """
        if not duration or duration <= MAX_TASK_DURATION_SECONDS:
            return
        minutes = duration / 60
        limit_minutes = MAX_TASK_DURATION_SECONDS / 60
        self._emit_event(
            task_id, "rejected", "视频超长已拒绝",
            f"源时长 {minutes:.1f} 分钟，超过 {limit_minutes:.0f} 分钟上限，未受理。",
            level="error",
            data={
                "checkpoint": checkpoint,
                "duration_seconds": round(duration, 3),
                "limit_seconds": MAX_TASK_DURATION_SECONDS,
                "error_code": REJECT_DURATION_LIMIT,
            },
        )
        raise VideoTaskRejected(
            f"视频时长 {minutes:.1f} 分钟，超过 {limit_minutes:.0f} 分钟上限，已拒绝处理。"
        )
    ...
    async def cancel_task(self, task_id: str) -> None:
        """Stop execution and pending persistence before authoritative deletion."""
        ...
        if running is not None:
            ...
            try:
                ...
                pass
        self._pending_db_states.pop(task_id, None)
        worker = self._persist_workers.pop(task_id, None)
