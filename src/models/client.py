...
# 方案 §4.2 坑 4：`dimensions` 必须**显式**传。不传时上游返回模型原生维度
# （`Qwen/Qwen3-VL-Embedding-8B` 实测为 4096）。该失败在测试里看不见 ——
# 全链路测试都把 HTTP stub 掉了。取值是**枚举白名单**
# （`[64,128,256,512,768,1024,1536,2048,2560,4096]`，不含 3072 / 3000，
# 传了报 `code 20015`），不是上限约束。
...
class ModelRelayClient:
    ...
    async def embed(self, texts: list[Any]) -> list[list[float]]:
        """向量化一批输入。

        阶段 4c 起，元素既可以是 ``str``（文本），也可以是 ``{"image": <data URI>}``
        （图片）—— 上游端点的 ``input`` 接受同质或混合列表；实测
        ``[{"image": data_uri}, "文本"]`` 返回 2 个向量且批内逐条独立编码、无串扰。
        注意图片必须是 data URI（带 ``data:image/...;base64,`` 前缀），
        裸 base64 会被上游以 20015 拒绝。
        """
